...
| Auth schema CLI | `@better-auth/cli` (via `npx`) | 1.4.x | **Versions independently of `better-auth`** — the latest CLI is 1.4.x even with core at 1.6, and it still emits correct 1.6 schema (incl. admin plugin fields). Run `pnpm auth:gen` (→ `npx @better-auth/cli@latest generate`). Commit the output; drizzle-kit owns migrations. |
